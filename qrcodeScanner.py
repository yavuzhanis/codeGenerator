...
def createQR(*args):
    ...
    if data:
        ...
        # QR kod boyutlarını orantılı hale getirin
        qr_width = 250  # QR kodun genişliğini istediğiniz değere ayarlayın
        qr_height = qr_width  # Genişlik ve yükseklik değerlerini aynı tutun veya farklı bir değer belirleyin
        resized_img = img.resize((qr_width, qr_height))
        tkimage = ImageTk.PhotoImage(resized_img)
        qr_canvas.delete('all')
        qr_canvas.create_image(frame1.winfo_width() // 2, frame1.winfo_height() // 2, anchor=tk.CENTER, image=tkimage)
...
qr_canvas.create_image(frame1.winfo_width() // 2, frame1.winfo_height() // 2, anchor=tk.CENTER, image=cover_img)
